...
pub trait Image {
    fn pull_image(&self, _ctx: &::ttrpc::TtrpcContext, _: super::image::PullImageRequest) -> ::ttrpc::Result<super::image::PullImageResponse> {
        Err(::ttrpc::Error::RpcStatus(::ttrpc::get_status(::ttrpc::Code::NOT_FOUND, "/grpc.Image/PullImage is not supported".to_string())))
    }
}

pub fn create_image(service: Arc<Box<dyn Image + Send + Sync>>) -> HashMap<String, Box<dyn ::ttrpc::MethodHandler + Send + Sync>> {
    let mut methods = HashMap::new();

    methods.insert("/grpc.Image/PullImage".to_string(),
                    Box::new(PullImageMethod{service: service.clone()}) as Box<dyn ::ttrpc::MethodHandler + Send + Sync>);
