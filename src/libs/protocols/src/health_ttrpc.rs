...
pub fn create_health(service: Arc<Box<dyn Health + Send + Sync>>) -> HashMap<String, Box<dyn ::ttrpc::MethodHandler + Send + Sync>> {
    let mut methods = HashMap::new();

    methods.insert("/grpc.Health/Check".to_string(),
                    Box::new(CheckMethod{service: service.clone()}) as Box<dyn ::ttrpc::MethodHandler + Send + Sync>);

    methods.insert("/grpc.Health/Version".to_string(),
                    Box::new(VersionMethod{service: service.clone()}) as Box<dyn ::ttrpc::MethodHandler + Send + Sync>);
